...
func getNumberOfBacklogOrders(orders [][]int) int {
	...
	for _, order := range orders{
		p, a, t := order[0], order[1], order[2]
		if t == 0{
			for a > 0 && len(sq) > 0 && p >= sq[0].p{
				q := heap.Pop(&sq).(pair)
				mn := min(a, q.a)
				a -= mn 
				q.a -= mn 
			...
			if a > 0{
				heap.Push(&bq, pair{-p, a})
			}
		}else{
			for a > 0 && len(bq) > 0 && p <= -bq[0].p{
				q := heap.Pop(&bq).(pair)
				mn := min(a, q.a)
				a -= mn
				q.a -= mn 
	...
	res := 0
	for sq.Len() > 0{
		res += heap.Pop(&sq).(pair).a
	}
	for bq.Len() > 0{
		res += heap.Pop(&bq).(pair).a
	} 
	return res % mod
