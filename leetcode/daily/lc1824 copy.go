...
func minSideJumps(a []int) int {
	...
	for i := 1 ; i < n ; i ++ {
		if a[i] != 1 {dp[i][0] = min(dp[i][0], dp[i - 1][0])}
		if a[i] != 2 {dp[i][1] = min(dp[i][1], dp[i - 1][1])}
		if a[i] != 3 {dp[i][2] = min(dp[i][2], dp[i - 1][2])}
		if a[i] != 1 {dp[i][0] = min(dp[i][0], min(dp[i][1], dp[i][2]) + 1)}
		if a[i] != 2 {dp[i][1] = min(dp[i][1], min(dp[i][0], dp[i][2]) + 1)}
		if a[i] != 3 {dp[i][2] = min(dp[i][2], min(dp[i][0], dp[i][1]) + 1)}
	} 
